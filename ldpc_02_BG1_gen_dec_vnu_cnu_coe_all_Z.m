...
%   Z       = rom_data(iii);                % 提升因子（并行度）
    ...
    LDPC_COLUMU_MAX_NUM = max(variable_ctrl);               %BG1:30， BG2:23
    LDPC_COLUMU_MIN_NUM = min(variable_ctrl);               %1
    VNU_DLY = LDPC_COLUMU_MAX_NUM + LDPC_COLUMU_MIN_NUM + 4;%BG1:35,  BG2:28
    % LDPC校验矩阵行中1的数目的最大值和最小值
    LDPC_ROW_MAX_NUM = max(check_ctrl);                     %BG1:19,  BG2:10
    LDPC_ROW_MIN_NUM = min(check_ctrl);                     %3 
    CNU_DLY = LDPC_ROW_MAX_NUM - LDPC_ROW_MIN_NUM + 10;     %BG1:26,  BG2:17
    SUM_TIME = length(barrel_shifter) + VNU_DLY + 8;        %BG1:359, BG2:233
    ...
    %% 将控制字写入到coe文件中
    % Ixc_Ixd共22位。Ix_rdvld：1位；Ix_rdaddr：9位；Ix_dvld一位；......
    Ixc_Ixd =...
...
%     fclose(ptr_Ix_c);
    ...
    %% 处理变量节点更新读取LLR控制字
    % Iz_a1：读取LLR缓存RAM地址，如第一列有31个1，RAM的地址都为0。
    Iz_c = Iyc;%读取LLR缓存RAM使能
    Iz_a1 = zeros(1,length(Iyc));
    ij = 0;
    for i = 1 : length(Iyc)
        if Iyc(i) == 1
            Iz_a1(i) = ij;
